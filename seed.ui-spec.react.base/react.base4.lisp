	    ...
	    (labels ((generate-cell (datum)
			 ...
			 (if (= 0 (@ rows length))
				 ...
				 (if (= "plain" (@ cells 0 ty 0))
				     ...
				     (labels ((find-extent (rows count index)
						(let ((index (if index index 0)))
						  (cl :rr rows)
						  (if (or (= 0 (@ rows length))
							  (= 0 count))
						      index
						      (if (not (= "undefined" (typeof (@ rows 0 0 br))))
							  (find-extent (chain rows 
									      (slice (@ rows 0 0 br)))
								       (1- count)
								       (+ index (@ rows 0 0 br)))
							  (find-extent (chain rows (slice 1))
								       (1- count)
								       (1+ index)))))))
				       (let ((empty-rows #())
					     (enclose-table (if is-outer-form
								(lambda (input) 
								  (chain self (render-table-body input t)))
								(lambda (input)
					                          ;(cl :cells cells)
								  (cl :inp input)
								  (chain self (render-sub-list2 (@ cells 0)
												input)))))
					     (sublist-extent (find-extent (chain (list (chain cells (slice 1)))
										 (concat (chain rows (slice 1))))
									  (@ cells 0 br))))
					 (if (= 14 (@ cells 0 ix)) 
					     (cl :cc cells rows
						 ;; (@ (chain (list (chain cells (slice 1)))
						 ;; 	   (concat (chain rows (slice 1 (@ cells 0 br)))))
						 ;;    length)
						 sublist-extent))
					 (process-rows
					  (chain rows (slice sublist-extent))
					  (chain output (slice 0 (1- (@ output length)))
						 (concat (list (chain
								...
								(concat
								 (enclose-table
								  (process-rows
								   (chain (list (chain cells (slice 1)))
									  (concat
									   (chain rows (slice 1 sublist-extent))))
