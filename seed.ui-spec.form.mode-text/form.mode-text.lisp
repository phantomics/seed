  ...
  (:get-initial-state
   ...
   (lambda (methods)
     (let* ((self this)
	    (to-grow (if (@ self props context working-system)
			 ...
			 (@ methods grow))))
       (cl :prt (@ self props) methods (@ self props context parent-system))
       (chain j-query
   	      (extend (create set-delta (lambda (value) (extend-state point-attrs (create delta value))))
   		      methods (create grow-branch (lambda (space meta callback)
						    (cl :id (@ self props) (@ self state data id))
   ...
   (lambda (next-props)
	...
	(chain self editor-instance code-mirror doc
       ...
       (commit
	(cl :commit
	    (create text (chain self editor-instance code-mirror doc (get-value)))
	    (@ self editor-instance))
     	(if (not (= true (@ next-props data meta locked)))
     	    (chain self state context methods (grow-branch (create text (chain self editor-instance code-mirror doc
									       (get-value)))
     							   (create save true)))))))
     )
