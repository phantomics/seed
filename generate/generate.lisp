...
(defun load-system-directory (directory-path)
  (flet ((check-name (file)
    ...
    (let ((files (uiop:directory-files directory-path)))
      (loop :for f :in files :when (check-name f) :do ;; (print (list :aaxx (package-name *package*)))
                                                      (load f)))))

;; (with-open-file (input f)
...
(defmacro seed (name &body props)
  (let* ((access    (rest (assoc :access props)))
         ...
         (branch    (intern (string (getf access :to-branch)) (package-name *package*)))
         (join      (intern (string (getf access :to-join))   (package-name *package*)))
         (of-system (intern (string (getf access :of-system)) (package-name *package*)))
         (defbranch (intern (string (gensym "DEFBRANCH"))     (package-name *package*)))
                   ...
                   (with-open-file (output file-path :direction :output :if-does-not-exist :create
                     ...
                     (if as-string (write-string new-value output)
                         (let ((*print-case* :downcase))
                           (write (package-name *package*) :stream output)
