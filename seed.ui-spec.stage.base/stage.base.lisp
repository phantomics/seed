...
;; (setq aa (macroexpand '(simple-branch-layout bla :omit (:stage :clipboard :history)
;; 			:adjunct (:clipboard :history)
;; 			:extend (:menu (stage-extension-menu-base)))))

;; (defmacro simple-branch-layout (branch-specs-symbol &key (omit nil) (adjunct nil) (extend nil))
;;   (let ((specs (gensym)) (prospec-main (gensym)) (prospec-adj (gensym)) (name (gensym)) (index (gensym))
;; 	(param-checks (gensym)) (stage-params (gensym)) (insert-fibo-points (gensym)) (secondary-controls (gensym))
...
;; 		  (if ,specs
;; 		      (if (or ,@(mapcar (lambda (item) (list 'eq name item))
;; 					omit))
			   ...
			   (cons `(,',meta (:body ,@(if ,secondary-controls (list :sub-controls)))
					   :if (:type :vista :ct 0 :fill :fill-branch :branch ,,name
						...
						:contextual-menu
						      (:format
						       ,,(if (getf extend :menu)
							     (macroexpand (list (first (getf extend :menu))
