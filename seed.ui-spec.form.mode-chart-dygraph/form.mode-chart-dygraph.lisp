...
|#
   ...
   (let ((draw-line (lambda (ctx ent chart)
		      ...
		      (let ((points (list (if (or (not (@ ent layer-points))
	 ...
	 (intersect-line (lambda (ent point callback)
			   (let ((line-points (list (chain self chart (to-dom-coords (@ ent points 0 0)
										     (@ ent points 0 1)))
						    (chain self chart (to-dom-coords (@ ent points 1 0)
										     (@ ent points 1 1)))))
				 (margin 8))
     ...
     (create line (create draw draw-line
	     ...
	     retrace-x (create draw (lambda (ctx ent chart)
				      ...
				      (setf (@ ctx stroke-style) "blue"
					    (@ ctx line-width) 1)
				      (let ((x-origin (chain self chart (to-dom-x-coord (if (< (@ ent points 0 0)
											       (@ ent points 1 0))
											    (@ ent points 0 0)
											    (@ ent points 1 0)))))
					    (x-interval (- (chain self chart (to-dom-x-coord (@ ent points 0 0)))
							   (chain self chart (to-dom-x-coord (@ ent points 1 0))))))
					(loop for ratio in (list 0 0.382 0.618 1)
					   do (let ((x-level (if (< (@ ent points 0 0) (@ ent points 1 0))
	     ...
	     retrace-y (create draw (lambda (ctx ent chart)
				      ...
				      (let ((x-origin (chain self chart (to-dom-x-coord (if (< (@ ent points 0 0)
											    ...
											    (@ ent points 0 0)
											    (@ ent points 1 0)))))
					    (y-origin (chain self chart (to-dom-y-coord (if (< (@ ent points 0 1)
											       (@ ent points 1 1))
											    (@ ent points 0 1)
											    (@ ent points 1 1)))))
					    (y-interval (- (chain self chart (to-dom-y-coord (@ ent points 0 1)))
							   (chain self chart (to-dom-y-coord (@ ent points 1 1))))))
					(loop for ratio in (list 0 0.236 0.382 0.500 0.618 0.764 1.0)
					   do (let ((y-level (if (< (@ ent points 0 1) (@ ent points 1 1))
		   ...
		   (loop for ent in (@ self ephemera entities-in-flux)
		      do (if (= 0 (@ ent points-in-flux length))
			     ...
			     (let* ((time-interval (- (@ chart raw-data_ 1 0) (@ chart raw-data_ 0 0)))
				  ...
				  do (setf (getprop ent "points" point) (list (@ event layer-x) (@ event layer-y))
