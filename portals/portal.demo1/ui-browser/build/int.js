...
function initializeDraggable(element, mode, inSeries) {
    ...
    if (handleContainer) {
        var _js16 = handleContainer.childNodes;
        var _js18 = _js16.length;
        for (var _js17 = 0; _js17 < _js18; _js17 += 1) {
            var n = _js16[_js17];
...
var drawLine = function (ctx, ent, chart, points) {
    ...
    ctx.beginPath();
    var points22 = points ? points : derivePoints(ent, chart);
    var circleRadius = 5;
    ctx.moveTo(points22[0][0], points22[0][1]);
    ctx.lineTo(points22[1][0], points22[1][1]);
    ctx.closePath();
    ctx.stroke();
    if (ent.inFlux && 'false' !== ent.inFlux) {
        var diffs = [[points22[0][0] - points22[1][0], points22[0][1] - points22[1][1]], [points22[1][0] - points22[0][0], points22[1][1] - points22[0][1]]];
        ...
        ctx.lineWidth = 0.5;
        ctx.beginPath();
        ctx.arc(points22[0][0], points22[0][1], 1, 0, Math.PI * 2, true);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(points22[1][0], points22[1][1], 1, 0, Math.PI * 2, true);
        ctx.fill();
        ctx.beginPath();
        ctx.arc(points22[0][0], points22[0][1], circleRadius, -Math.PI + (ri[0] - Math.PI * 0.2), -Math.PI + (ri[0] + Math.PI * 0.2), true);
        ctx.stroke();
        ctx.beginPath();
        ctx.arc(points22[1][0], points22[1][1], circleRadius, -Math.PI + (ri[1] - Math.PI * 0.2), -Math.PI + (ri[1] + Math.PI * 0.2), true);
        ctx.stroke();
    };
    ctx.strokeStyle = 'black';
    __PS_MV_REG = [];
...
function interactorMousewheel(mode) {
    return function (event, chart, context) {
        if (event.shiftKey) {
            ...
            var zoomInterval29 = 0.05 * timeInterval;
            var wheelDelta30 = 0 < event.deltaY ? 1 : -1;
            return chart.updateOptions({ dateWindow : [1 === wheelDelta30 ? timeRange[0] - zoomInterval29 : timeRange[0] + zoomInterval29, timeRange[1]], valueRange : chart.yAxisRange() });
        };
    };
};
function interactorMousedown(mode) {
    return function (event, g, context) {
        ...
        case 'select':
            ...
            var _js31 = entitiesCount - 1;
            for (var entix = 0; entix <= _js31; entix += 1) {
        ...
        if (mode.mousedown) {
            ...
            case 'select':
                if (0 === mode.entitiesInFlux.length) {
                    ...
                    var movingFrom38 = mode.movingFrom;
                    tempCanvas.clearRect(0, 0, chart.canvas_.width, chart.canvas_.height);
                    var _js39 = mode.entitiesInFlux;
                    var _js41 = _js39.length;
                    for (var _js40 = 0; _js40 < _js41; _js40 += 1) {
                        var ent = _js39[_js40];
                        if (0 === ent.pointsInFlux.length) {
                            ent.layerPoints = [[ent.layerPoints[0][0] - (movingFrom38[0] - event.layerX), ent.layerPoints[0][1] - (movingFrom38[1] - event.layerY)], [ent.layerPoints[1][0] - (movingFrom38[0] - event.layerX), ent.layerPoints[1][1] - (movingFrom38[1] - event.layerY)]];
                        } else {
            ...
            case 'draw':
                var timeInterval42 = chart.rawData_[1][0] - chart.rawData_[0][0];
                var domCoords43 = chart.eventToDomCoords(event);
                var dataPos44 = chart.toDataCoords(domCoords43[0], domCoords43[1]);
                var remainder45 = (dataPos44[0] % timeInterval42 + timeInterval42) % timeInterval42;
                var ent46 = mode.activeEntity;
                if (0 !== remainder45) {
                    dataPos44[0] -= remainder45;
                };
                ent46.points[1] = [dataPos44[0], dataPos44[1]];
                tempCanvas.clearRect(0, 0, chart.canvas_.width, chart.canvas_.height);
                return drawMethods[ent46.type]['draw'](tempCanvas, ent46, chart);
            };
        };
    };
};
        ...
        if (0 !== e.seriesIndex) {
            ...
            if (4 !== setCount) {
                ...
                var barCount = (range = e.dygraph.xAxisRange(), counting = false, length = 0, ((function () {
                    var _js47 = sets[0];
                    var _js49 = _js47.length;
                    for (var _js48 = 0; _js48 < _js49; _js48 += 1) {
                        var point = _js47[_js48];
                ...
                ctx.lineWidth = 0.6;
                var _js50 = sets[0].length - 1;
                for (var p = 0; p <= _js50; p += 1) {
